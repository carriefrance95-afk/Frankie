  ...
  const openMessage = async (message: EmailMessage) => {
    ...
    try {
      ...
      if (account.provider === 'zoho') {
        if (!message.folderId) {
          throw new Error(
            'Zoho did not return the folder information needed to open this email.',
          )
        }

        params.set('folderId', message.folderId)
      }

      const endpoint =
        account.provider === 'zoho'
          ? '/api/zoho/email-message-detail'
          : '/api/google/email-message-detail'
      ...
      <div className="email-workspace-scroll">
        ...
        !isLoadingAccounts ? (
          <div className="email-empty-state">
            <span>✉</span>
            <h3>No email inbox is connected.</h3>
            <p>
